...
export default function index__Quotes() {
	...
	return (
		<Tank fluid className={classnames("quotes", "pos_" + quotePosition)}>
			<img className="quote_sign" src={quote_sign} alt="" height="30" width="30" />
			{Object.values(allQuotes).map((q, k) => {
				return (
					<blockquote className={classnames("Tank", k === quotePosition ? "active" : "")} key={k}>
						<p className="font_size_2_4 font-light font-serif">{q.quote}</p>
						<span className={classnames("cite", q.id)}></span>
					</blockquote>
				);
			})}
			<div className="quote_num">
				<span style={{ color: "#FFF" }}>{(quotePosition + 1).toString().padStart(2, "0")}</span>/{allQuotesQty.toString().padStart(2, "0")}
			</div>
